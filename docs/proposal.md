...
本專案目標是用 Python 實作一個可在桌面環境執行的雙人對戰方塊遊戲，玩法以經典 Tetris 與 Facebook 時代的 Tetris Friends Battle 2P 類型對戰為參考。專案不使用官方商標、圖片、音效或受版權保護的素材；需求中的「Tetris」僅作為玩法類型描述。
...
公開資料顯示，Facebook 時代的 Tetris Battle / Tetris Friends Battle 2P 具有下列特徵，可作為本專案的目標參考：

- 遊戲盤面為 10 x 20 可見格。
- 使用現代 guideline 類規則，包含 SRS 旋轉、hold、hard drop、next queue 等功能。
- Battle 2P 是 2 分鐘制。Tetris Friends Battle 2P 的目標是先 KO 對手 3 次；若時間結束仍未達成，勝負通常依 KO 數與送出垃圾行數等條件判定。
- 對戰壓力來自垃圾行。玩家透過消多行、連續消行 combo、T-spin、back-to-back 等行為對對手送出垃圾。
- Facebook 版 Tetris Battle 的常見垃圾形式包含 bomb garbage；Arena 類即時模式則更接近傳統有洞垃圾行。

本專案對戰模式決定採用 Tetris Friends Battle 2P 風格：兩人限時 2 分鐘；任一玩家先取得 3 次 KO 可提前獲勝；若時間結束未達 3 KO，依 KO 數、送出垃圾行數與後續設計定義的 tie-breaker 判定勝負。MVP 垃圾型態採傳統有洞 garbage，不採 bomb garbage。攻擊表、垃圾抵銷、延遲套用與 hole 位置策略的精確規則，應在後續設計階段定版。
...
- 對戰體驗以 Tetris Friends Battle 2P 風格的限時 KO 對戰為目標，MVP 採傳統有洞 garbage。
...
server 允許額外玩家排隊等待。每一局只有兩名 active players；輸掉的一方離開 active match，等待隊列中的下一名玩家進入下一局。active players 以外的等待隊列容量必須可配置，MVP 預設為 5 名等待玩家；超過容量時 server 應拒絕加入並回傳房間已滿訊息。
...
- FR-VS-002: 對戰回合目標採 Tetris Friends Battle 2P 風格：限時回合，透過送垃圾與 KO 對手取得勝利。
- FR-VS-003: MVP 回合時間目標為 2 分鐘；實作需允許後續配置。
- FR-VS-004: 玩家 top-out 時視為被 KO。被 KO 的玩家應在同一回合中重生並繼續遊玩；任一玩家先取得 3 次 KO 時提前獲勝。
- FR-VS-005: 若回合時間結束且無人達成 3 次 KO，勝負判定優先順序目標為：KO 數、送出垃圾行數、結束盤面高度。精確 tie-breaker 在設計階段定義。
...
- FR-VS-009: MVP 垃圾型態採傳統有洞 garbage。bomb garbage 不屬於 MVP，但後續可作為可選規則或模式擴充。
...
- FR-NET-006: server 必須允許等待隊列容量配置；MVP 預設支援 5 名非 active 等待玩家。若 active match 已滿且等待隊列也已滿，server 必須拒絕新玩家並回傳房間已滿訊息。
...
- FR-UDP-006: 網路同步策略應優先降低本地操作延遲與維持遊戲手感。設計階段應在 client 本地模擬搭配 server 事件轉發，或 client 本地模擬搭配 server snapshot 校正的混合式方案中定版，同時保留可測試與可除錯的協定邊界。
...
- FR-GUI-008: GUI framework 採 PySide6。GUI 美術風格與具體 widget / rendering 分工留待設計階段確認，但目標是比基本工具視窗更精緻、具有完整遊戲介面的視覺品質。
...
- 完整 FB 版所有攻擊數值精準還原。
- bomb garbage 邏輯；MVP 採傳統有洞 garbage。
...
- PySide6 會影響跨平台打包難度，需在設計階段評估 packaging 工具、Qt plugin 收集與平台建置流程。
- Tetris Friends Battle 2P 風格的精確攻擊數值、垃圾抵銷、延遲套用與有洞 garbage 位置策略需要進一步校準；需求階段只定義目標行為。
...
- PySide6 GUI 架構與遊戲畫面 rendering 方式。
- 遊戲核心資料模型。
- SRS / randomizer / lock delay 的精確規格。
- Tetris Friends Battle 2P 風格攻擊表與傳統有洞垃圾行規則。
- 低延遲優先的 UDP 協定與同步模型。

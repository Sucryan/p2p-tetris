...
- [ ] `common`：共用設定、ID 與時間接口完成，獨立測試通過。
- [ ] `game-core`：單人核心規則完成，snapshot 與 deterministic engine 可測。
- [ ] `battle-rules`：attack、garbage、KO、respawn、winner resolution 純邏輯完成。
- [ ] `controllers`：keyboard、scripted 與 future RL action source 抽象完成。
- [ ] `client-runtime`：單人與對戰 session orchestration、view model 與 fake net 串接完成。
- [ ] `client-gui`：PySide6 GUI、screen flow、game renderer 與 runtime wiring 完成。
- [ ] `net`：UDP protocol、codec、reliability、endpoint 與 loss/duplicate fixture 完成。
- [ ] `server`：session、queue、match lifecycle、車輪戰與 UDP 串接完成。
- [ ] `packaging`：entrypoints、Linux 打包 automation、Windows 打包文件與本機操作文件完成。
- [ ] `tests`：共用 fixture、整合測試、GUI smoke test 與品質檢查命令完成。
